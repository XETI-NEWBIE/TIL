...
# 왜 A = list(map(int, input().split()))으로 묶어줘야 하는가??

# 1. map은 "게으른" 녀석이에요 (Lazy Evaluation)
# 파이썬 3에서 map 객체는 그 자체로 리스트가 아닙니다.

# map의 상태: "자, 내가 int로 바꿀 준비는 다 해놨어. 근데 네가 하나씩 달라고 하기 전까지는 실제로 바꾸지는 않을게!"라고 대기하는 상태예요.

# 이걸 출력해보면 [1, 2, 3]이 나오는 게 아니라 <map object at 0x...> 같은 이상한 주소값이 나옵니다.

# 2. 왜 list()로 감싸야 하나요?
# 우리가 A = list(map(...))라고 쓰는 이유는 **"야, 공장 그만 돌리고 결과물 한꺼번에 다 뽑아서 리스트(바구니)에 담아줘!"**라고 명령하는 거예요.

# 인덱싱 때문: A[0], A[b_idx]처럼 인덱스로 특정 위치의 값을 읽으려면, 데이터가 메모리에 쫙 깔려 있는 리스트 형태여야만 합니다. 

# map 객체는 [0]번으로 접근이 안 돼요.

# 재사용 때문: map은 한 번 훑고 지나가면 다시 쓸 수 없지만, list로 만들어두면 두고두고 쓸 수 있습니다.