...
선택 정렬은 전체 원소 중 최솟값을 찾아 정해진 위치로 이동시키는 방식입니다.

**작동 원리** : 외부 루프의 인덱스 $i$는 최솟값이 들어갈 **'확정석'**을 의미합니다. 내부 루프 $j$는 $i$의 다음 칸부터 마지막 칸까지 순회하며 실제 최솟값이 위치한 인덱스(min_idx)를 찾아냅니다. 루프가 끝나면 단 한 번의 교환(swap)을 통해 해당 자리에 최솟값을 배치합니다.
...
# 📊 버블 정렬 vs 선택 정렬 핵심 비교

> **💡 한 줄 요약**
>
> - **버블 정렬**: 인접한 두 개를 계속 비교하며 **"큰 놈을 뒤로"** 보낸다.
> - **선택 정렬**: 전체 중 최솟값을 찾아 맨 앞과 교환하며 **"작은 놈을 앞으로"** 보낸다.
...
- **작동 원리**: 인접한 두 원소를 비교하여 큰 값을 뒤로 교환.
- **정렬 순서**: **뒤쪽**부터 하나씩 확정됨.
- **특징**: 코드가 직관적이지만 교환(`swap`)이 너무 자주 일어남.

```python
for i in range(N - 1, 0, -1):  # 1. 정렬 구간의 끝(i)을 줄여나감
    for j in range(i):         # 2. 처음부터 i까지 옆집끼리 비교
        if a[j] > a[j + 1]:    # 3. 앞이 더 크면 자리 바꿈 (오름차순)
            a[j], a[j + 1] = a[j + 1], a[j]
```

## 2. 🎯 선택 정렬 (Selection Sort)

"범인(최솟값) 한 놈만 골라서 맨 앞으로!"

작동 원리: 남은 구간에서 최솟값을 찾아 정렬되지 않은 맨 앞자리와 교환.

정렬 순서: **앞쪽(First)**부터 하나씩 확정됨.

특징: 버블 정렬보다 교환 횟수가 훨씬 적어서 상대적으로 효율적임.

```
for i in range(N - 1):            # 1. 숫자가 들어갈 '자리'를 정함
    min_idx = i                   # 2. 일단 시작점을 최솟값으로 가정
    for j in range(i + 1, N):     # 3. 그 뒤(i+1)부터 끝까지 뒤져서
        if a[min_idx] > a[j]:     # 4. 진짜 최솟값의 위치를 찾아냄
            min_idx = j
    a[i], a[min_idx] = a[min_idx], a[i]  # 5. 마지막에 딱 한 번만 교환!
```

## 3. 결정적 차이점 (시험 필수 암기)

### 🏁 버블 정렬 vs 선택 정렬 결정적 차이점

| 비교 항목     | 버블 정렬 (Bubble)           | 선택 정렬 (Selection)           |
| :------------ | :--------------------------- | :------------------------------ |
| **핵심 동작** | 옆집과 계속 교체             | 최솟값 찾아 한 번에 교체        |
| **정렬 방향** | 큰 값이 **뒤(Right)**로 이동 | 작은 값이 **앞(Left)**으로 이동 |
| **교환 횟수** | **많음** (비효율적)          | **적음** (상대적 효율적)        |
| **범위 제어** | `j`는 항상 `0`부터 출발      | `j`는 항상 `i + 1`부터 출발     |
